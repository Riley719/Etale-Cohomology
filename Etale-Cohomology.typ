...
前層から無理やり層を作る層化という操作がある。

#statementsp(
  box-name: "prop",
  box-title: "Sheafification",
  box-label: "sheafification",
  number: true
)[
  $X$を位相空間として$cal(F) in Psh(X, cal(C))$を前層とする。このとき，次の普遍性を満たす層$a cal(F)$と自然な前層の射$i_cal(F) colon cal(F) --> a cal(F)$が存在する。
  $
    #align(center, diagram({
      node((-1, -1), [$cal(F)$])
      node((-1, 0), [$a cal(F)$])
      node((0, -1), [$forall cal(G)$])
      edge((-1, -1), (-1, 0), [$i_cal(F)$], label-side: right, "->")
      edge((-1, -1), (0, -1), "->")
      edge((-1, 0), (0, -1), "-->")
    }))
  $
  つまり，任意の前層$cal(F)$と，層$cal(G)$について次の自然同型（随伴）がある。
  $
    Hom_(Sh(X, cal(C)))(a cal(F), cal(G)) cong Hom_(Psh(X, cal(C)))(cal(F), i cal(G)) 
  $
  $
    #align(center, diagram({
      node((0, -1), [$Sh(X, cal(C))$])
      node((1, -1), [$Psh(X, cal(C))$])
      edge((0, -1), (1, -1), [$i$], label-side: left, "->", bend: 18deg)
      edge((1, -1), (0, -1), [$a$], label-side: left, "->", bend: 18deg)
    }))
  $
  $i$は忘却関手。$a cal(F)$を層化という。
]

テンソル積の構成と同じで，層化の正体は知らなくていいと思う。大事なのは，上で述べた随伴と，次に述べるストークが変わらないということ。その前に，$cal(F)$という前層と$forall x in X$に対して
$
  cal(F)_x = colim_(x in U) cal(F)(U)
$とさ
とと定め，これを$cal(F)$の点$x$における*stalk*と呼ぶ。$U$は開近傍すべてを渡る。これにより
$
  Psh(X, cal(C)) --> cal(C) : cal(F) mapsto.long cal(F)_x
$
という関手を得る。($cal(C)$は余極限が存在するとしてしまっている。)この関手を$S_x$と書くことにする。反対に，さらに$cal(C)$に終対象$0$が存在するとして，次のような関手を考えられる。
$
  cal(C) --> Psh(X, cal(C)) : C mapsto.long x_*(C)
$
ただし
$
  x_*(C)(U) = C (x in U), quad 0 (x in.not U)
$
と定める。(摩天楼層関手)\
実はこれらは随伴である。つまり次が成り立つ。
$
  Hom_(cal(C))(S_x (cal(F)), C) cong Hom_(Psh(X, cal(C)))(cal(F), x_*(C))
$
以上を用いて次のことが言える。前層$cal(F)$の点$x$におけるstalkは層化しても変わらない。つまり

#statementsp(
  box-name: "prop",
  box-title: "",
  box-label: "stalkofsheafifi",
  number: true
)[
  $X$を位相空間として$cal(F) in Psh(X, cal(C))$を前層とする。$forall x in X$に対して次の同型がある。
  $
    (a cal(F))_x cong cal(F)_x
  $
  つまり，層化の前後でストークは変わらない。
]
#pfsp[
  任意に$E in cal(C)$をとる。
  $
    Hom_(cal(C))(S_x (a cal(F)), E) &cong Hom_(Psh(X, cal(C)))(a cal(F), x_*(E)) \
    &cong Hom_(Psh(X, cal(C)))(cal(F), x_*(E)) \
    &cong Hom_(cal(C))(S_x (cal(F)), E)
  $
  である。米田の定理から$S_x (a cal(F)) cong S_x (cal(F))$がわかる。
]

また，層の射の単射性と全射性はstalkのそれに帰着できる。層化でstalkが変わらないこと，単射，全射はstalkで判定できることはよく使う。(本当ならこれの同型バージョンを先に示していた方が厳密だがめんどうなので割愛。)
...
#statementsp(
  ...
  $phi colon cal(F) --> cal(G)$を位相空間$X$上の層の射とする。次は同値。

  + $phi$は単射（resp. 全射）
  + 任意の$x in X$に対して$phi_x colon cal(F)_x --> cal(G)_x$は単射（resp. 全射）
]
#pfsp[
  まず単射について示す。\
  $1 => 2$: $phi_x (s_x) = 0$となる$s_x in cal(F)_x$をとる。$s_x = 0$を示す。$s_x$は$x$の十分小さな開近傍$x in U$と$s in cal(F)(U)$の自然な射$cal(F)(U) --> cal(F)_x$の像である。$phi_x (s_x) = 0$なので$phi_U (s)_x = 0$である。ゆえに$x in V subset U$なる開集合をとって$phi_U (s)|_V = 0$である。よって$phi_U (s)|_V = phi_V (s|_V) = 0$だから仮定から$s|_V = 0$である。ゆえに$s_x = 0$である。\
  $2 => 1$: 任意に開集合$U$をとって$phi_U (s) = 0$なる$s in cal(F)(U)$をとる。ここで任意に$x in U$をとる。
  $
    phi_x (s_x) = phi_U (s)_x = 0 in cal(G)_x
  $
  なので，仮定から$s_x = 0 in cal(F)_x$。よって$x in V_x subset U$なる開集合をとって$s|_V_x = 0 in cal(F)(V_x)$。被覆$U = union.big V_x$と$0 = s|_V_x in cal(F)(V_x)$であること，貼り合わせの公理とその一意性から$s = 0$である。

  次に全射性について示す。\
  $1 => 2$: imageを対応させる前層の層化$a(im(phi))$について，$a(im(phi)) cong cal(G)$が存在するという仮定である。(層の射が全射というのはこういう定義。)この両辺を$x$でstalkをとると
  $
    cal(F)_x --> cal(G)_x cong a(im(phi))_x cong im(phi)_x cong im(phi_x)
  $
  となり，全射である。\
  $2 => 1$: $cal(G)_x cong a(im(phi))_x$なので層の射としても$cal(G) cong a(im(phi))$となり示せる。
]
...
ずいぶん天下りに見えるが，これは天から降ってきたものではない。次に述べるようにKAN拡張とその計算で定義が見えてくる。

KAN拡張については#link("https://alg-d.com/math/kan_extension/kan_extension.pdf")[alg-dのPDF]を参照。\
左右どちらのKAN拡張もとることができるが，どちらも計算してみよう。正解は右KAN拡張であるから，先に右をやる。\
次の図式を考える。
$
  #align(center, diagram({
    node((-1, 0), [$"Op"(X)^("op")$])
...
$
$"Op"(X)$や$cal(B)$は小圏なので$i^("op")$に沿った$cal(F)$の右KAN拡張$cal(F)^("ext")$が図式のように存在する。こうしてPreSheafを得ることができる。どんなPreSheafなのか計算しよう。KAN拡張はコンマ圏を用いた各点右KAN拡張で計算できる。コンマ圏はこれまた#link("https://alg-d.com/math/kan_extension/comma.pdf")[alg-dのPDF]を参照。つまり
$
  "Ran"_(i^("op")) (cal(F)) (U) = lim(U arrow.b i^("op") --> cal(B)^("op") stretch(-->)^(cal(F)) cal(C))
$
となっている。$U arrow.b i^("op")$はコンマ圏であり，対象は$(*, B, f)$という三つ組みで，$*$は自明な圏$bb(1)$の唯一の対象，$B in cal(B)^("op")$，$f colon U --> B$ in $"Op"(X)^("op")$の射である。$* mapsto.long U$という関手と$i^("op")$でコンマ圏を作ったということ。このコンマ圏の対象であることと$U supset B$であることは同値である。それで逆極限をとるから，結局
$
  "Ran"_(i^("op")) (cal(F)) (U) = lim_(B subset U, B in cal(B)) cal(F)(B)
$
である。証明で述べたようにこれはSheafをなしているから欲していた$cal(F)^("ext")$である。

では左KAn拡張を選んでしまうとどうなるだろうか。途中までは一緒だが，次のようになる。
$
  "Lan"_(i^("op")) (cal(F)) (U) = colim_(U subset B, B in cal(B)) cal(F)(B)
$
一見よさそうに見えるが，これでは層にならないかもしれない。例えば$U$によっては$U subset B$となる$B in cal(B)$が存在しないかもしれない。添え字圏が空圏になってしまう。このとき，圏の余極限の定義を思い出すと，この余極限は$cal(C)$の始対象であることが要請される。$cal(C)$が始対象をもたないときはこの余極限は存在しないことになる。$cal(C)$が始対象をもつときはこの余極限は存在してそれになるが，例えば$cal(C)$がアーベル圏なら$0$を返すことになり，どう考えてもSheafにならなそうである。だからこっちは失敗なのだ。
...
このように，最終的にはアフィンの議論に帰着されるということはよくある。これをメタ定理として整備するというのは今後ネータースキーム以外のスキームを定義するときに便利である。@SPAut18 の#link("https://stacks.math.columbia.edu/tag/01OO")[01OO]に載っている。
...
#statementsp(
  ...
  $PP$を環に対する条件とする。(例えばネーター，CM，Gorenstein，Regularなど。)$PP$がlocalであるとは，次の二つを満たすときに言う。
  + 任意の環$R$と$f in R$について，$PP(R) ==> PP(R_f)$
  + 任意の環$R$とと$(f_1, dots, f_n) = R$なる元について，$(forall i, PP(R_(f_i))) ==> PP(R)$
]

#statementsp(
  box-name: "def",
  box-title: "",
  box-label: "locschemeprop",
  number: true
)[
  $PP$を環に対する条件とする。$X$をスキームとする。$X$が*locally $PP$*であるとは，任意の点$x in X$に対して，あるアフィン開近傍$Spec(A)$であって$A$が$PP$を満たすものが存在するときをいう。
]
...
#statementsp(
  box-name: "lem",
  box-title: "Affine Communication Lemma for Schemes",
  box-label: "communisch",
  number: true
)[
  $X$をスキームとして$PP$を環の条件とする。次は同値。
  + $X$はlocally $PP$である。
  + 任意のアフィン開集合$U$に対して$Gamma(U, cal(O)_X)$が$PP$を満たす。
  + あるアフィン開被覆$X = union.big_(i in I) U_i$が存在して，任意の$i$に対して$Gamma(U_i, cal(O)_X)$が$PP$を満たす。
  + ある開被覆$X = union.big_(i in I) X_i$が存在して，$X_i$が部分スキームとしてloccally $PP$である。
  さらに，$X$がlocally $PP$であるとき，その任意の開部分スキームもlocally $PP$である。
]
#pfsp[
  (1) $<==> (3)$は明らか。(2) $==>$ (1)も明らか。(3) $==>$ (2)が示されれば，最後の主張が正しいと分かる。すると(1) $<==>$ (4)もわかる。ゆえに(3) $==>$ (2)を示せばいい。\
  $X = union.big_(i in I) U_i$をアフィン開被覆で$U_i = Spec(R_i)$とし，$R_i$は$PP$を満たすとする。$U = Spec(R)$を任意のアフィン開集合とする。#linksp(<lem:standardopen>)より，$U$は$D(f)$という形の$R$の基本開集合であって，ある$i$に対して$D(f) = Spec((R_i)_(g))$という形のもので被覆できる。$R_i$は$PP$を満たすので$R_f$も$PP$を満たす。$U$は準コンパクトだから，このような被覆の有限部分被覆をとれる。これにて$R$が$PP$を満たすことが示された。
]

これがあるので，環の性質をスキームにもっていくとき，上の補題のような言い換えを示したかったら，環のレベルで#linksp(<def:locprop>)の条件を満たすことを示せばいい。ネーター，CM，Gorenstein，Regularなどの性質はlocalであることが知られているので，これらの性質をスキームに持っていくときにはこの補題を使える。
...
#statementsp(
  box-name: "lem",
  box-title: "Affine Communication Lemma for Morphisms of Schemes",
...
$
は前層になる。これの層化を$cal(Hom)_(cal(O)_X) (cal(F), cal(G))$と書くことにする。

同じく，$cal(F), cal(G)$に対して
$
  U mapsto.long cal(F)(U) tensor_(cal(O)_X (U)) cal(G)(U)
$
は層になる。これを$cal(F) tensor_(cal(O)_X) cal(G)$と書くことにする。

$cal(F) cong cal(O)_X^I$となるとき，$cal(F)$は自由であるという。$cal(F)$が局所自由であるとは，任意の点$x in X$に対して，ある開近傍$U$が存在して，$cal(F)|_U cong cal(O)_X|_U^I$となることをいう。つまり局所的には自由であるということ。このとき，開集合$U$上の$cal(F)$のランクとは，$I$と同型な順序数を指す。もし$X$が連結なら，$cal(F)$のランクはどの開近傍で見ても同じになる。ランクが1になるような局所自由層を可逆層という。

$X$上のイデアル層とは，$cal(F)$という加群層で，$cal(O)_X$の部分層である。言い換えれば任意の開集合$U$に対して$cal(F)(U)$は$cal(O)_X (U)$のイデアルである。

スキームの射$(f, f^sharp) colon (X, cal(O)_X) --> (Y, cal(O)_Y)$に対して，$cal(F)$を$cal(O)_X$加群があるとする。このとき$f_*cal(F)$は$f_*cal(O)_X$加群となる。つまり$Y$の開集合$U$について$cal(F)(f^(-1)(U))$は$cal(O)_X (f^(-1)(U))$加群となる。$cal(O)_X (f^(-1)(U))$は$cal(O)_Y (U)$代数なので，$cal(F)(f^(-1)(U))$は$cal(O)_Y (U)$加群になる。
$
  cal(O)_Y (U) stretch(-->)^(f^sharp_U) cal(O)_X (f^(-1)(U)) stretch(-->) cal(F)(f^(-1)(U))
$
という流れで$f_*cal(F)$は$cal(O)_Y$加群になる。これを$cal(F)$の$f$による順像という。

なぜ順像かというと，$f colon X --> Y$というスキームの射があり，定義域の$X$に対して加群層$cal(F)$がある状況で，$f$によって$cal(F)$をうしろの$Y$に移すからである。$f$という射の方向と同じ方向に情報を流すから順像。

逆像を定義する。$f colon X --> Y$をスキームの射とし，$cal(G)$を$cal(O)_Y$加群とする。$f^(-1)cal(G)$という$cal(O)_X$加群を定義する。それは$X$の開集合$U$に対して
$
  U mapsto.long colim_(f(U) subset V) cal(G)(V)
$
という前層の層化である。同様に$f^(-1)cal(O)_Y$を作ることもできる。$f^(-1)cal(G)$は$f^(-1)cal(O)_Y$加群である。さらに$f^(-1)cal(O)_Y$は$cal(O)_X$代数なので，$f^(-1)cal(G)$は$cal(O)_X$加群になる。よって$f^*cal(G) = f^(-1)cal(G) tensor_(f^(-1)cal(O)_Y) cal(O)_X$と定義できる。これも$cal(O)_X$加群になる。これを$cal(G)$の$f$による逆像という。なぜ逆像というかというと，$f colon X --> Y$というスキームの射があり，後ろの$Y$に対して加群層$cal(G)$がある状況で，$f$によって$cal(G)$を前の$X$に移すからである。$f$という射の方向と逆の方向に情報を流すから逆像。

$f colon X --> Y$をスキームの射とすると，順像と逆像は$cal(O)_X$加群の圏と$cal(O)_Y$加群の圏の随伴を与える。つまり
$
  Hom_(cal(O)_X) (f^*cal(G), cal(F)) cong Hom_(cal(O)_Y) (cal(G), f_*cal(F))
$
が成立する。
...
#statementsp(
  ...
  $A$を環として$M$を$A$加群とする。$Spec(A)$の基本開集合$D(f)$に対して$M_f$を割り当てる操作は層になると分かる。これを任意の開集合に拡張してできる層を$tilde(M)$とかく。
]

$tilde(M)$は$cal(O)_Spec(A)$加群になる。また，前層としての対応は
$
  U mapsto.long lim_(D(f) subset U) M_f
$
である。特に
$
  D(f) mapsto.long M_f
$
である。stalkは$M_frak(p)$となる。つまり$(tilde(M))_frak(p) = M_frak(p)$である。

#statementsp(
  box-name: "def",
  box-title: "(quasi) coherent sheaves",
  box-label: "coherent",
  number: true
)[
  $(X, cal(O)_X)$をスキームとする。$cal(O)_X$加群$cal(F)$がquasi coherentであるとは，任意の$x in X$に対して，あるアフィン開近傍$x in U = Spec(A)$が存在して$cal(F)|_U = tilde(M)$となるような$A$加群$M$が存在することをいう。

  $cal(F)$がcoherentであるとは，次の二つを満たすときをいう。
  + $cal(F)$は有限型，つまり任意の$x in X$に対してある開近傍$x in U$をとってさらに$n$が存在し，$cal(O)_X|_U^n --> cal(F)|_U --> 0$が完全になる。
  + 任意の開集合$U$と$s_i in cal(F)(U)$に対して，$cal(O)_X|_U^n --> cal(F)|_U$を$e_i mapsto.long s_i$である写像とすると，これのkernelが有限型。

  2つめについて，開集合$V subset U$に対して
  $
    cal(O)_X|_U (V)^n in.rev (v_1, dots, v_n) mapsto.long v_1 s_1|_V + dots.c + v_n s_n|_V in cal(F)|_U (V)
  $
  という写像である。
]

準連接層の圏を$"QCoh"(cal(O)_X)$とかく。連接層の圏を$"Coh"(cal(O)_X)$とかく。@Har77 だと連接層の定義は少し異なる。そこでは準連接層の定義で$M$を有限生成にすればいい，と書いてある。これは一般には我々の連接層とは異なる。しかし$X$がネータースキームなら同じ概念である。

準連接の概念は$A$上の加群の概念と次の意味で一致する。

#statementsp(
  box-name: "th",
  box-title: "",
  box-label: "equivofmod",
  number: true
)[
  $A$を環とする。$A$加群の圏と準連接$cal(O)_Spec(A)$加群の圏は圏同値である。つまり，次の自然な関手が存在して，それは互いに共変圏同値である。
  $
    tilde(("-")) &colon A"-Mod" --> "QCoh"(cal(O)_Spec(A)) colon M mapsto tilde(M)\
    Gamma(Spec(A), ("-")) &colon "QCoh"(cal(O)_Spec(A)) --> A"-Mod" colon cal(F) mapsto Gamma(Spec(A), cal(F))
  $
  上を層化関手，下を大域切断関手という。
]
これらが加法的なのは明らかなので，これらは完全関手でもある。証明は@上野05 のどこかに載ってた。射の対応は次の通りである。まずは$tilde(("-"))$について。
$
  (phi colon M --> N) mapsto.long (tilde(phi) colon tilde(M) --> tilde(N))
$
ただし$tilde(phi)$は層の間の射，つまり自然変換であり，開集合$U$に対しては
$
  tilde(phi)_U colon lim_(D(f) subset U) M_f --> lim_(D(f) subset U) N_f
$
という$phi$から誘導される写像をとる。次に$Gamma(Spec(A), ("-"))$について。
$
  (psi colon cal(F) --> cal(G)) mapsto.long (Gamma(Spec(A), psi) colon Gamma(Spec(A), cal(F)) --> Gamma(Spec(A), cal(G)))
$
これは単に一番上の加群の射を持ってくればいいだけ。

一般に，アーベル圏の間の関手がなにかの左随伴であるとき，余極限を保つ。だから右完全関手である。(テンソルはHomの左随伴であった。これは右完全であった。このことを思い出せ。)同様に，アーベル圏の間の関手がなにかの右随伴であるとき，極限を保つ。だから左完全関手である。圏同値である関手は左右随伴であるから完全関手である。実は圏同値なら加法的になることもわかる。

また
$ 
  Hom_(cal(O)_X)(f^*(tilde(M)), cal(G))
  &cong Hom_(cal(O)_Y)(tilde(M), f_* cal(G)) \
  &cong Hom_A (M, Gamma(Y, f_* cal(G))) \
  &cong Hom_A (M, Gamma(X, cal(G))) \
  &cong Hom_A (M, Hom_B (B, Gamma(X, cal(G)))) \
  &cong Hom_B (M tensor_A B, Gamma(X, cal(G))) \
  &cong Hom_(cal(O)_X)(tilde(M tensor_A B), cal(G))
$
というように計算でき，米田の補題から$f^*(tilde(M)) = tilde(M tensor_A B)$とわかる。

#statementsp(
  box-name: "prop",
  box-title: "",
  box-label: "quasicohanotherdef",
  number: true
)[
  $X$をスキームとして$cal(F)$を$cal(O)_X$加群とする。$cal(F)$が準連接層であることと，任意の$x in X$に対し，ある開近傍$x in U$が存在し，次の完全列が存在することは同値である。
  $
    cal(O)_X|_U^(plus.o J) --> cal(O)_X|_U^(plus.o I) --> cal(F)|_U --> 0
  $
]
#pfsp[
  まず，$cal(F)$が準連接層であるとする。任意に$x in X$をとる。定義より$x in U = Spec(A)$というアフィン開近傍と$A$加群$M$であって$cal(F)|_U = tilde(M)$となるものがある。加群$M$について，次のような自由分解を考えることができる。
  $
    A^(plus.o J) --> A^(plus.o I) --> M --> 0
  $
  これに$tilde$をつける。
  $
    cal(O)_X|_U^(plus.o J) --> cal(O)_X|_U^(plus.o I) --> cal(F)|_U --> 0
  $
  これの各点でのstalkは完全列になる。なぜならstalkは局所化であるからである。よってこの層の列は完全列である。

  逆に，任意の$x in X$に対してある開近傍$x in U$が存在し，次の完全列が存在するとする。
  $
    cal(O)_X|_U^(plus.o J) stretch(-->)^phi cal(O)_X|_U^(plus.o I) --> cal(F)|_U --> 0
  $
  必要ならさらに$U$を絞ってアフィンとしてよく，$U = Spec(A)$とおく。これに$Gamma(Spec(A), "-")$を作用させる。すると
  $
    A^(plus.o J) stretch(-->)^(Gamma(U, phi)) A^(plus.o I)
  $
  を得る。これのcokerを$M$として
  $
    A^(plus.o J) stretch(-->)^(Gamma(U, phi)) A^(plus.o I) --> M --> 0
  $
  を得る。これに$tilde$をつける。
  $
    cal(O)_X|_U^(plus.o J) stretch(-->)^(tilde(Gamma(U, phi))) cal(O)_X|_U^(plus.o I) --> tilde(M) --> 0
  $
  を得る。これは完全列。$tilde(Gamma(U, phi))$は$phi$と同じであることがすぐにわかる。よって$cal(F)|_U = tilde(M)$がわかる。(cokerは一意)よって$cal(F)$は準連接層である。
]

これより連接層ならば準連接層であることがわかる。というのは，連接層の定義の一つ目から任意の$x in X$に対してある開近傍（必要なら小さくしてアフィンとしていい）$U$をとって
$
  cal(O)_X|_U^n --> cal(F)|_U --> 0
$
を完全とできる。2つめからこれのkernelは有限型なので，結局
$
  cal(O)_X|_U^(m) --> cal(O)_X|_U^n --> cal(F)|_U --> 0
$
という完全列を得る。ゆえに準連接。
...
#statementsp(
  ...
  $X$を局所ネータースキームとする。このとき，$cal(O)_X$加群$cal(F)$が準連接の有限生成バージョン，つまり任意の$x in X$に対してある開近傍$x in U$が存在し，$cal(F)(U)$が有限生成$cal(O)_X (U)$加群であるなら，$cal(F)$は連接層である。この逆も成立する。
]
#pfsp[
  逆は明らか。順方向は任意に$x in X$をとる。定義よりある開近傍$x in U$が存在して，$cal(F)(U)$は有限生成$cal(O)_X (U)$加群である。この時点ではじめからアフィンスキームで考えていいことがわかる。つまり，$X = Spec(A)$で$A$はネーター環，$cal(F) = tilde(M)$で$M$は有限生成$A$加群としていい。ネーター環上有限生成加群の部分加群はまた有限生成加群なので
  $
    A^m --> A^n --> M --> 0
  $
  という分解が取れる。これに$tilde$をつける。
  $
    cal(O)_X^m --> cal(O)_X^n --> cal(F) --> 0
  $
  を得る。これで連接層の1つ目の条件は満たされる。2つ目の条件も同様にネーター環上有限生成加群の部分加群は有限生成加群，というのを使えばできる。つまり，任意の開集合$V$をとって
  $
    cal(O)_X|_V^n --> cal(F)|_V
  $
  という写像を考える。$cal(F)|_V = tilde(N)$とおく。これの大域切断を考えて
  $
    Gamma(V, cal(O)_X|_V)^n --> Gamma(V, cal(F)|_V) = N
  $
  となる。これのkerを$k$とおく。
  $
    0 --> k --> Gamma(V, cal(O)_X|_V)^n --> N
  $
  を得る。これに$tilde$をつける。
  $
    0 --> tilde(k) --> cal(O)_X|_V^n --> cal(F)|_V
  $
  を得る。ゆえにもとの$cal(O)_X|_V^n --> cal(F)|_V$のkernelは$tilde(k)$であることがわかる。$k$は有限生成加群だから
  $
    cal(O)_X|_V^m --> k --> 0
  $
  とできる。これに$tilde$をつける。
  $
    cal(O)_X|_V^m --> tilde(k) --> 0
  $
  である。ゆえにkernelも有限型。これで2つ目の条件も満たされる。よって$cal(F)$は連接層である。
]

== Cohomology of Sheaves
まず復習のため，$"Ext"_R^i (M,N)$の作り方を説明する。これにはまず，$N$のinjective resolutionをとる。
$
  0 --> N --> I^0 --> I^1 --> dots.c
$
これに$Hom_R (M,"-")$を作用させる。
$
  0 --> Hom_R (M, I^0) --> Hom_R (M, I^1) --> dots.c
$
このcomplexの$i$次コホモロジーをとる。
$
  H^i (Hom_R (M, I^bullet)) = im(Hom_R (M, I^(i-1)) --> Hom_R (M, I^i))/ker(Hom_R (M, I^i) --> Hom_R (M, I^(i+1)))
$
これを$"Ext"_R^i (M,N)$と書くのであった。

一般に，左完全加法的関手$F$に対して，同じ手法が使える。つまり，まず$N$のinjective resolutionをとる。
$
  0 --> N --> I^0 --> I^1 --> dots.c
$
これに$F$を作用させる。
$
  0 --> F(I^0) --> F(I^1) --> dots.c
$
このcomplexの$i$次コホモロジーをとる。
$
  H^i (F(I^bullet)) = im(F(I^(i-1)) --> F(I^i))/ker(F(I^i) --> F(I^(i+1)))
$
これを$R^i F(N)$と書く。これが右導来関手である。

さて，$cal(C)$をアーベル群の圏とする。これはアーベル圏であってenough injectiveである。実は$Sh(X, cal(C))$もまたアーベル圏であってenough injectiveである。そして，関手$Gamma(X, "-")$は左完全加法的関手である。ゆえに上の方法でコホモロジーを定義できる。これによって出現したコホモロジーを*層係数コホモロジー*という。つまり次のようである。

まずスキーム$X$と$cal(O)_X$加群$cal(F)$を用意する。まず$cal(F)$のinjective resolutionをとる。
$
  0 --> cal(F) --> cal(E)^0 --> cal(E)^1 --> cal(E)^2 --> dots.c
$
$cal(E)^i$たちは$"Mod"(cal(O)_X)$の中でのinjective objects，つまり$Hom_("Mod"(cal(O)_X)) ("-", cal(E)^i)$という$"Mod"(cal(O)_X)$から$cal(O)_X (X)$加群の圏への反変関手が完全であるような$cal(O)_X$加群である。これに$Gamma(X, "-")$を施す。
$
  0 --> Gamma(X, cal(F)) --> Gamma(X, cal(E)^0) --> Gamma(X, cal(E)^1) --> Gamma(X, cal(E)^2) --> dots.c
$
これの$i$番目のコホモロジーを$H^i (X, cal(F))$と書いて，$cal(O)_X$加群$cal(F)$の$i$番目の層係数コホモロジーというのである。

コホモロジーが消えるような層は重要である。そこで次を定義する。
...
#statementsp(
  ...
  $X$をスキームとして$cal(F)$を$cal(O)_X$加群とする。$cal(F)$がflasqueであるとは，任意の開集合$U subset V$に対して制限写像$"res"_(U,V)^(cal(F)) : cal(F)(V) --> cal(F)(U)$が全射であることをいう。
]

一見コホモロジーとは関係ないが，いずれこのクラスの層はコホモロジーが消えることがわかる。ゆえに重要なクラスである。次は@Har77 のp.67の演習である。演習のくせに本文で使っているしむずくね，と思う。

#statementsp(
  box-name: "lem",
  box-title: "",
  box-label: "exactflasque",
  number: true
)[
  $X$をスキームとして次の$cal(O)_X$加群の完全列を考える。
  $
    0 --> cal(F) stretch(-->)^(phi) cal(G) stretch(-->)^(psi) cal(H) --> 0
  $
  このとき，$cal(F)$と$cal(G)$がflasqueであるならば，$cal(H)$もflasqueである。
]
完全列ということなので，例えば$im(phi)$などは$cal(F)$のimageを対応させる前層の層化である。層化をとるとよくわからなくなると思ってしまうが，stalkは変わらないというところをうまく使う。

#pfsp[
  まず，$cal(F)$がflasqueであることのみを使って，任意に開集合$V$をとったときに$cal(G)(V) --> cal(H)(V)$が全射になることを示す。任意に$h in cal(H)(V)$をとる。次の集合を考えよう。
  $
    Sigma = {(W, g) mid(|) W subset V : "open", g in cal(G)(W) "and" psi_W (g) = h|_W}
  $
  $(emptyset, 0)$があるのでこれは空でない。この集合に順序を
  $
    (W, g) <= (W', g') quad <==> quad W subset W' : "open", g'|_W = g
  $
  と定める。するとこれはZornの補題を満たす。(上限のセクションは層の張り合わせを用いる。)ゆえに極大元$(W, g)$が存在する。$W = V$を示す。もし$W subset.neq V$であるなら，$x in V \\ W$をとれる。完全列から$cal(G)_x --> cal(H)_x$は全射である。よって$h_x in cal(H)_x$という像に飛んでくる$g_x in cal(G)_x$が存在する。$g_x$はある開集合$x in U$と$u in cal(G)(U)$という元で代表される。構成から$psi_U (u)$をstalk $cal(H)_x$に送ると$h_x$になる。$h$と$psi_U (u)$はともに$cal(H)_x$で同じ元にいくから$x in W_x$という開集合であって$psi_U (u)|_W_x = h|_W_x$とできる。必要なら$W_x$や$U$を小さくとって，$x in W_x$という開集合と$g' in cal(G)(W_x)$という元で$g$を$psi$で飛ばすと$h|_W_x$に飛ぶようにできる。ここで
  $
    g|_(W inter W_x) - g'|_(W inter W_x) mapsto.long h|_(W inter W_x) - h|_(W inter W_x) = 0
  $
  となる。再び完全性から
  $
    cal(F)(W inter W_x) in.rev exists r mapsto.long g|_(W inter W_x) - g'|_(W inter W_x) in cal(G)(W inter W_x)
  $
  とできる。$cal(F)$はflasqueだから
  $
    cal(F)(W_x) in.rev exists r' mapsto.long r in cal(F)(W inter W_x)
  $
  とできる。ここで$cal(G)(W_x)$の中で
  $
    t = g' + phi_W_x (r')
  $
  を考えることができる。
  $
    t = g' + phi_W_x (r') mapsto.long h|_W_x + 0 = h|_W_x
  $
  であるので$(W_x, t) in Sigma$である。一方$(W, g) in Sigma$について考えると
  $
    t|_(W inter W_x) = g'|_(W inter W_x) + phi_W_x (r')|_(W inter W_x) = g'|_(W inter W_x) + phi_(W inter W_x) (r) = g|_(W inter W_x)
  $
  となるから，貼り合わせによって$g'' in W union W_x$という関数を構成できる。$(g'', W union W_x) in Sigma$を確認できるが，取り方から$W subset.neq W union W_x$なので，極大性に矛盾。よって$W = V$である。これより$psi_V (g) = h|_V = h$だから$cal(G)(V) --> cal(H)(V)$は全射。

  ここまで，任意の開集合について
  $
    0 --> cal(F)(V) stretch(-->)^(phi_V) cal(G)(V) stretch(-->)^(psi_V) cal(H)(V) --> 0
  $
  も完全であることを示した。最後に$cal(H)$がflasqueであることを示す。任意に$V subset U$をとる。$cal(H)(U) --> cal(H)(V)$が全射なことを示す。任意に$s in cal(H)(V)$をとる。前示したことから$cal(G)(V) in.rev exists t mapsto.long s in cal(H)(V)$をとれる。$cal(G)$はflasqueだから$cal(G)(U) in.rev exists t' mapsto.long t in cal(G)(V)$をとれる。この$t'$の像を$s' in cal(H)(U)$とする。これは明らかに制限で$s$に飛ぶ。よってflasque。
]

$Hom_(cal(O)_X) ("-", cal(I))$が$cal(O)_X$-Modの圏からアーベル群の圏への(反変)完全関手となるとき，$cal(I)$を*injective $cal(O)_X$加群*という。

#statementsp(
  box-name: "lem",
  box-title: "",
  box-label: "injisflasque",
  number: true
)[
  injectiveな$cal(O)_X$加群はflasqueである。
]
#pfsp[
  任意の$X$の開集合$U$に対して$j_! (cal(O)_X|_U)$と書いたら次のような$X$上の前層の層化とする。
  $
    V mapsto.long cases(
      cal(O)_X (V) quad &(V subset U),
      0 quad &(V subset.not U)
    )
  $
  この前層は明らかに$cal(O)_X$の部分層である。よって各stalkをとると単射なのでstalkは層化の前後で変わらないことを考えれば
  $
    0 --> j_! (cal(O)_X|_U) --> cal(O)_X
  $
  は層の完全列。$cal(I)$をinjectiveな$cal(O)_X$加群とすると
  $
    Hom_cal(O_X) (cal(O)_X, cal(I)) --> Hom_cal(O_X) (j_! (cal(O)_X|_U), cal(I)) --> 0
  $
  は完全列。実は次の随伴がある。
  $
    Hom_cal(O)_X (j_! (cal(O)_X|_U), cal(I)) = Hom_cal(O)_U (cal(O)_X|_U, cal(I)|_U) = cal(I)(U)
  $
  となる。最後は米田の補題で示せると思う。ゆえに
  $
    cal(I)(X) --> cal(I)(U) --> 0
  $
  という全射があることになり，示せた。
...
ここまでの話を合わせるとflasqueな対象はacyclic，つまり1次以上の層係数コホモロジーで消滅することがわかる。

#statementsp(
  box-name: "th",
  box-title: "flasqueならコホモロジーは消滅",
  box-label: "flasquevanish",
  number: true
)[
  $X$をスキームとして$cal(F)$をflasqueな$cal(O)_X$加群とする。このとき$H^i (X, cal(F)) = 0 quad (forall i>0)$となる。
]
#pfsp[
  $cal(F)$をまずはinjectiveな層$cal(I)$に埋め込む。
  $
    0 --> cal(F) --> cal(I)
  $
  これの剰余層を$cal(G)$と書けば
  $
    0 --> cal(F) --> cal(I) --> cal(G) --> 0
  $
  という完全列を得る。$cal(F)$はflasqueで#linksp(<lem:injisflasque>)より$cal(I)$もflasque。ゆえに#linksp(<lem:exactflasque>)によって$cal(G)$もflasqueである。$cal(F)$がflasqueだから同じ補題の証明から
  $
    0 --> Gamma(X, cal(F)) --> Gamma(X, cal(I)) --> Gamma(X, cal(G)) --> 0
  $
  は完全である。ゆえに$H^1 (X, cal(F)) = 0$である。一方$H^i (X, cal(I))$はすべて$0$である。このことと$H^1 (X, cal(G)) = 0$を合わせて（ここまで，flasqueなら1次コホモロジーが消えることを示せている。）以下の長完全列から結果を得る。
  $
    #align(center, diagram({
      node((-1, -1), [$0$])
      node((0, -1), [$Gamma(X, cal(F))$])
      node((1, -1), [$Gamma(X, cal(I))$])
      node((2, -1), [$Gamma(X, cal(G))$])
      node((0, 0), [$H^1 (X, cal(F))$])
      node((1, 0), [$H^1 (X, cal(I))$])
      node((2, 0), [$H^1 (X, cal(G))$])
      node((0, 1), [$H^2 (X, cal(F))$])
      node((1, 1), [$H^2 (X, cal(I))$])
      node((2, 1), [$H^2 (X, cal(G))$])
      node((0, 2), [$dots.c$])
      edge((-1, -1), (0, -1), "->")
      edge((0, -1), (1, -1), "->")
      edge((1, -1), (2, -1), "->")
...
次の補題を示すことでネーターアフィンスキーム上の準連接層の層係数コホモロジーが消えることを示せる。

#statementsp(
  box-name: "lem",
  box-title: "",
  box-label: "injmoduleisflasque",
  number: true
)[
  $E$をネーター環$A$上のinjective moduleとする。このとき$X = Spec(A)$上の準連接層$tilde(E)$はflasqueである。
]
#pfsp[
  omit
]

次の定理はアフィン性をコホモロジーで記述できるセールによる定理である。上の補題と#linksp(<th:equivofmod>)を用いることで(1)ならば(2)は示せる。(2)ならば(3)は自明。(3)ならば(1)は難しい。@Har77 だとネータースキームであることは本質的であるような証明になっている。しかしネーターは次のように必要ない。(1)ならば(2)ではチェックコホモロジーを使うとこれが示せるが準備が足りないのでここでは示せない。

#statementsp(
  box-name: "th",
  box-title: "Serre's Criterion",
  box-label: "serrecriterion",
  number: true
)[
  $X$をquasi-compactなスキームとする。次は同値。

  #enum(
    numbering: "(1)",
    enum.item(1)[$X$はアフィンスキーム],
    enum.item(2)[任意の準連接層$cal(F)$と$i > 0$に対して$H^i (X, cal(F)) = 0$],
    enum.item(3)[任意の準連接的なイデアル層$cal(I)$に対して$H^1 (X, cal(I)) = 0$]
  )
]

== Local Cohomology
このセクションでは局所コホモロジー$H_I^i (M)$と層係数コホモロジーが適切なセッティングの元では一致するという定理を示しに行く。

次は@Har77 p.67のII.Ex.2.14である。
...
#statementsp(
  ...
  $(X, cal(O)_X)$をSchemeとする。$cal(F)$を$cal(O)_X$加群とする。開集合$U$とsection $s in cal(F)(U)$に対して
  $
    Supp(s) = {x in U | s_x eq.not 0 "in" cal(F)_x}
  $
  と定める。これを$cal(F)$における$s$の*support*と呼ぶ。ただし$s_x$は点$x$におけるstalk $cal(F)_x$における$s$の像である。
]
...
#pfsp[
  補集合が開集合なことを示す。つまり
  $
    U \\ Supp(s) = {x in U | s_x = 0 "in" cal(F)_x}
  $
  が開集合なことを示す。$x in U \\ Supp(s)$とする。$s_x = 0$である。
  $
    cal(F)_x = colim_(x in V) cal(F)(V)
  $
...
#statementsp(
  ...
  $(X, cal(O)_X)$をSchemeとする。$cal(F)$を$cal(O)_X$加群とする。$Z subset X$を閉集合とし，
  $
    Gamma_Z (X, cal(F)) = {s in Gamma(X, cal(F)) | Supp(s) subset Z}
  $
  と定める。
  $
    X supset V mapsto.long Gamma_(Z inter V) (V, cal(F)|_V)
  $
  と対応を定めると，これは$cal(O)_X$加群になる。これを*$Z$にsupportをもつ$cal(F)$の部分$cal(O)_X$加群*と呼び，$cal(H)_Z^0(cal(F))$とかく。
]
...
#statementsp(
  ...
  #linksp(<def:suppsch>)で定めた対応は$cal(O)_X$加群になる。
]
#pfsp[
  まず，任意の開集合$U$に対して
  $
    Gamma_(Z inter U) (U, cal(F)|_U) = {s in Gamma(U, cal(F)|_U) | Supp(s) subset Z inter U}
  $
  が$cal(O)_X (U)$加群になることを示す。$s, t$をこの集合の元とすると，$Supp(s + t) subset Supp(s) union Supp(t) subset Z inter U$であるから$s + t$もこの集合の元である。加法の逆元で閉じていることは明らか。これが$cal(O)_X (U)$加群になることも明らか。

  Sheafになることを示す。Presheafになることはよい。任意の開集合$U$とその開被覆$U = union.big_(i in I) U_i$をとる。$s in Gamma_(Z inter U) (U, cal(F)|_U)$であって$s|_(U_i) = 0$とする。$cal(F)$がSheafなので$s = 0$である。

  $s_i in Gamma_(Z inter U_i) (U_i, cal(F)|_U_i)$とし，任意の$i,j$に対して$s_i|_(U_i inter U_j) = s_j|_(U_i inter U_j)$であるとする。$cal(F)$がSheafなので，ただひとつ貼り合わせ$s in cal(F)(U) supset Gamma_(Z inter U) (U, cal(F)|_U)$をとれる。これがさらに$Gamma_(Z inter U) (U, cal(F)|_U)$に入ることを確認すればいい。$Supp(s) subset Z inter U$を示す。任意に$x in Supp(s)$をとる。$x in U_i$となる$i$をとれる。すると
  $
...
実はこうして得た$cal(O)_X$加群を考えることで，これが局所コホモロジーとの同型が示される。まずはこの層を与える対応が左完全なことを示さなければならない。
...
#statementsp(
  ...
  $X$をスキーム，$Z$をその閉集合とする。このとき$Gamma_Z (X, "-")$は$cal(O)_X$加群の圏から$cal(O)_X (X)$加群の圏への左完全関手である。
]
#pfsp[
  $
    0 --> cal(F) stretch(-->)^(phi) cal(G) stretch(-->)^psi cal(H) --> 0
  $
  を$cal(O)_X$加群の完全列とする。
  $
    0 --> Gamma_Z (X, cal(F)) --> Gamma_Z (X, cal(G)) --> Gamma_Z (X, cal(H))
  $
  が完全なことを示す。まず，各射がちゃんと得られること，つまり$phi colon cal(F) --> cal(G)$があったときに$phi_X colon Gamma(X, cal(F)) --> Gamma(X, cal(G))$の制限によって$phi_X colon Gamma_Z (X, cal(F)) --> Gamma_Z (X, cal(G))$が得られることを確認する。任意に$s in Gamma_Z (X, cal(F))$をとる。$Supp(s) subset Z$である。$Supp(phi_X (s)) subset Z$を確認すれば$phi_X (s) in Gamma_Z (X, cal(G))$がわかる。任意に$x in Supp(phi_X (s))$をとる。$0 eq.not (phi_X (s))_x = phi_x (s_x)$であるから$s_x eq.not 0$でなければならない。つまり$x in Supp(s)$だから$x in Z$となり示せた。

  左の射の単射性は$Gamma(X, cal(F)) --> Gamma(X, cal(G))$が単射なことから明らか。中心の完全性を示す。非自明なのは$ker(psi_X) subset im(phi_X)$である。任意に$s in ker(psi_X)$をとる。$Gamma(X, "-")$の左完全性は既知なので$exists t in Gamma(X, cal(F))$であって$phi_X (t) = s$なるものは取れる。$t in Gamma_Z (X, cal(F))$を示そう。$Supp(t) subset Z$を示せればいい。任意に$x in Supp(t)$をとる。$t_x eq.not 0$である。$phi_x$は単射なので$phi_x (t_x) eq.not 0$である。$0 eq.not phi_x (t_x) = (phi_X (t))_x = s_x$なので$x in Supp(s) subset Z$となり示せた。
]

ということで$Gamma_Z (X, "-")$の右導来関手としてのコホモロジーを$H_Z^i (X, cal(F))$とかく。
...
#statementsp(
  ...
  $X$をスキームとし$Z$をその閉集合とする。$cal(F)$を$cal(O)_X$加群とする。次が成立する。

  + $0 --> cal(F) stretch(-->)^phi cal(G) stretch(-->)^psi cal(H) --> 0$は$cal(F)$がflasqueな完全列とする。このとき$0 --> Gamma_Z (X,cal(F)) --> Gamma_Z (X, cal(G)) --> Gamma_Z (X, cal(H)) --> 0$は完全列。
  + $cal(F)$がflasqueなら$i>0$に対し$H_Z^i (X, cal(F)) = 0$
  + $cal(F)$がflasqueなら$0 --> Gamma_Z (X, cal(F)) --> Gamma(X, cal(F)) --> Gamma(X\\Z, cal(F)) --> 0$は完全列。
  + $U = X\\Z$とする。次の長完全列がある。
    $
      #align(center, diagram({
        node((-1, -1), [$0$])
        node((0, -1), [$H_Z^0 (X, cal(F))$])
        node((1, -1), [$H^0 (X, cal(F))$])
        node((2, -1), [$H^0 (U, cal(F)|_U)$])
        node((0, 0), [$H_Z^1 (X, cal(F))$])
        node((1, 0), [$H^1 (X, cal(F))$])
        node((2, 0), [$H^1 (U, cal(F)|_U)$])
        node((0, 1), [$H_Z^2 (X, cal(F))$])
        node((1, 1), [$H^2 (X, cal(F))$])
        node((2, 1), [$H^2 (U, cal(F)|_U)$])
        node((0, 2), [$dots.c$])
        edge((-1, -1), (0, -1), "->")
        edge((0, -1), (1, -1), "->")
        edge((1, -1), (2, -1), "->")
    ...
    $
  + $V$を$Z subset V$なる開集合とする。このとき自然な同型$H_Z^i (X, cal(F)) cong H_Z^i (V, cal(F)|_V)$がある。
]
#pfsp[
  + このとき次が全射なことは既に#linksp(<lem:exactflasque>)で示した。
    $
      0 --> Gamma(X, cal(F)) --> Gamma(X, cal(G)) --> Gamma(X, cal(H)) --> 0
    $
    ゆえに，任意に$s in Gamma_Z (X, cal(H))$をとると$t in Gamma(X, cal(G))$であって$psi_X (t) = s$となるものは取れる。また，同じく#linksp(<lem:exactflasque>)から$U = X\\Z$に対して
    $
      0 --> Gamma(U, cal(F)) --> Gamma(U, cal(G)) --> Gamma(U, cal(H)) --> 0
    $
    は完全。ゆえに
    $
      psi_U (t_U) = psi_X (t)|_U = s|_U = 0
    $
    である。($U$の任意の点でのstalkでは$s_x = 0$だから$s|_U = 0$)よって$r in Gamma(U, cal(F))$であって$psi_U (r) = t$なるものが取れる。$cal(F)$はflasqueだから$u in Gamma(X, cal(F))$であって$u|_U = r$なるものがある。ここで$t' = t - phi_X (u)$と定める。明らかに$psi_X (t') = s$である。$t' in Gamma_Z (X, cal(G))$を示す。$Supp(t') subset Z$を示せばいい。任意に$x in U$をとる。$t'_x = t_x - (psi_X (u))_x = (psi_U (r))_x - ((psi_X (u))|_U)_x = (psi_U (r))_x - (psi_U (r))_x = 0$である。ゆえに$x in.not Supp(t')$。つまり$x in.not Z ==> x in.not Supp(t')$が示された。よって$Supp(t') subset Z$。
  + #linksp(<th:flasquevanish>)と同様。
  + 左が単射なのは明らかで，右が全射なのは$cal(F)$がflasqueだからである。
    $
      0 --> Gamma_Z (X, cal(F)) stretch(-->)^i Gamma(X, cal(F)) stretch(-->)^("res") Gamma(X\\Z, cal(F)) --> 0
    $
    よって$im(i) = ker("res")$を示さなければならない。$im(i) subset ker("res")$を示す。$s in Gamma_Z (X, cal(F))$をとり，これを$U = X\\Z$に制限したら$0$になることを示せばいいが，これは明らか。反対の包含を示す。$s in Gamma(X, cal(F))$をとって$s|_U = 0$とする。このとき$Supp(s) subset Z$を示せばいいがこれも明らか。
  + $0 --> cal(F) --> cal(I)^0 --> cal(I)^1 --> cal(I)^2 --> dots.c$をinjective resolutionとする。3により次の可換で横が完全な複体を得る。
    $
      #align(center, diagram({
        node((-1, -1), [$0$])
        node((0, -1), [$0$])
        ...
        node((0, 3), [$dots.v$])
        node((1, 3), [$dots.v$])
        node((-1, 0), [$Gamma_Z (X, cal(I)^0)$])
        node((0, 0), [$Gamma (X, cal(I)^0)$])
        node((1, 0), [$Gamma (U, cal(I)^0)$])
        node((-1, 1), [$Gamma_Z (X, cal(I)^1)$])
        node((-1, 2), [$Gamma_Z (X, cal(I)^2)$])
        node((0, 1), [$Gamma (X, cal(I)^1)$])
        node((0, 2), [$Gamma (X, cal(I)^2)$])
        node((1, 1), [$Gamma (U, cal(I)^1)$])
        node((1, 2), [$Gamma (U, cal(I)^2)$])
        edge((-1, -1), (-1, 0), "->")
        edge((-1, 0), (-1, 1), "->")
        edge((-1, 1), (-1, 2), "->")
        edge((0, -1), (0, 0), "->")
    ...
    $
    ホモロジー代数の長完全列の補題を用いることで所望の完全列を得る。
  + （スケッチ）制限写像によって$Gamma_Z (X, cal(F)) --> Gamma_Z (V, cal(F))$が取れることを確認し，これが同型なことを示す。これが$cal(F)$に関して関手的なのは明らかなので高次のコホモロジー，つまり導来関手が一致することがわかる。
]
...
#statementsp(
  ...
  $X = Spec(A)$を$A$がネーター環となるスキーム，$Z = V(frak(a))$をイデアル$frak(a)$による$X$の閉集合とする。任意の$A$加群$M$に対して$A$加群としての自然な同型
  $
    H_frak(a)^i (M) cong H_Z^i (X, tilde(M))
  $
  がある。
]
#pfsp[
  $Gamma_frak(a) ("-")$と$Gamma_Z (X, tilde(("-")))$の間に自然な同型があることを示す。それぞれ$A$加群の圏からそれ自身への関手であって
  $
    Gamma_frak(a) (M) &= {m in M mid(|) exists n in NN "s.t." frak(a)^n m = 0}\
    Gamma_Z (X, tilde(M)) &= {s in tilde(M)(X) = M mid(|) Supp(s) subset Z = V(frak(a))}
  $
  である。$x in X$は$A$の素イデアルであることに注意する。$s in M$について
  $
    Supp(s) = {x in X mid(|) s_x = 0} = {frak(p) in Spec(A) mid(|) s_frak(p) = 0} = V("Ann"(s))
  $
  が成り立つ。よって
  $
    Supp(s) subset Z &<==> V("Ann"(s)) subset V(frak(a))\ 
    &<==> sqrt("Ann"(s)) supset sqrt(frak(a))\ 
    &<==> exists n in NN "s.t." "Ann"(s) supset frak(a)^n
  $
  $frak(a)$は有限生成だから最後の同値が成立する。よって$Gamma_frak(a) (M) = Gamma_Z (X, tilde(M))$である。ここで$M$のinjective resolututionをとる。
  $
    0 --> M --> I^0 --> I^1 --> I^2 --> dots.c
  $
  これを準連接化する。
  $
    0 --> tilde(M) --> tilde(I^0) --> tilde(I^1) --> tilde(I^2) --> dots.c
  $
  これも完全列であり，#linksp(<lem:injisflasque>)によってこれはflasque resplution。flasqueはacyclicであり，右導来関手はacyclic resolutionをとってもいいので
  $
    #align(center, diagram({
      node((-1, -1), [$0$])
      node((-1, 0), [$0$])
      node((0, -1), [$Gamma_Z (X, tilde(M))$])
      node((1, -1), [$Gamma_Z (X, tilde(I^0))$])
      node((3, -1), [$Gamma_Z (X, tilde(I^2))$])
      node((2, -1), [$Gamma_Z (X, tilde(I^1))$])
      node((0, 0), [$Gamma_frak(a) (M)$])
      node((1, 0), [$Gamma_frak(a) (I^0)$])
      node((2, 0), [$Gamma_frak(a) (I^1)$])
      node((3, 0), [$Gamma_frak(a) (I^2)$])
      node((4, -1), [$dots.c$])
      node((4, 0), [$dots.c$])
      edge((-1, -1), (0, -1), "->")
      edge((0, -1), (1, -1), "->")
      ...
      edge((3, -1), (4, -1), "->")
      edge((3, 0), (4, 0), "->")
      edge((4, -1), (4, 0), [$dots.c$], label-side: center, " ")
    }))
  $
  が成り立つ。ゆえに各コホモロジーは同型。
]

実はこれも$A$がネーター環というのは外せる。ただし$frak(a)$が有限生成イデアルとしないと成り立たないことがある。(有限生成は必要条件ではないと思うが。)

後半のresolutionをとる部分でチェック複体を代わりに用いることでネーターを外したバージョンの証明が可能になる。

この定理で$frak(a) = 0$とすると，当然局所コホモロジー側は自明なことをやっているとすぐにわかる。一方で右辺は層係数コホモロジーになる。このことからもSerreの判定法の成立がわかる。

局所コホモロジーのとき同様，次のような完全列も有用である。

#statementsp(
  box-name: "prop",
  box-title: "Mayer-Vietoris Exact Sequence",
  box-label: "",
  number: true
)[
  $X$をスキームとして$Z_1, Z_2$を$X$の閉集合とする。$cal(F)$を$cal(O)_X$加群とする。このとき次の完全列がある。
  $
    #align(center, diagram({
      node((-1, -1), [$0$])
      node((0, -1), [$H_(Z_1 inter Z_2)^0 (X, cal(F))$])
      node((1, -1), [$H_(Y_1)^0 (X, cal(F)) plus.o H_(Y_2)^0 (X, cal(F))$])
      node((2, -1), [$H_(Z_1 union Z_2)^0 (X, cal(F))$])
      node((0, 0), [$H_(Z_1 inter Z_2)^1 (X, cal(F))$])
      node((0, 1), [$H_(Z_1 inter Z_2)^2 (X, cal(F))$])
      node((1, 0), [$H_(Y_1)^1 (X, cal(F)) plus.o H_(Y_2)^1 (X, cal(F))$])
      node((1, 1), [$H_(Y_1)^2 (X, cal(F)) plus.o H_(Y_2)^2 (X, cal(F))$])
      node((2, 0), [$H_(Z_1 union Z_2)^1 (X, cal(F))$])
      node((2, 1), [$H_(Z_1 union Z_2)^2 (X, cal(F))$])
      node((0, 2), [$dots.c$])
      edge((-1, -1), (0, -1), "->")
      edge((0, -1), (1, -1), "->")
      edge((1, -1), (2, -1), "->")
...
#pfsp[
  $cal(F)$がflasqueな場合に，次の完全列が存在することを示せれば，残りはホモロジー代数の一般論で仕上がる。
  $
    0 --> Gamma_(Z_1 inter Z_2) (X, cal(F)) stretch(-->)^alpha Gamma_(Z_1) (X, cal(F)) plus.o Gamma_(Z_2) (X, cal(F)) stretch(-->)^beta Gamma_(Z_1 union Z_2) (X, cal(F)) --> 0
  $
  ただし$alpha(s) = (s, -s), beta(s_1, s_2) = s_1 + s_2$である。このような射が定まることと$alpha$が単射なこと，$im(alpha) subset ker(beta)$であることは明らかである。$ker(beta) subset im(alpha)$を示す。$(s_1, s_2) in ker(beta)$を示す。$s_1 = -s_2$であり，$s_1 in Gamma_(Z_1) (X, cal(F))$であるので$Supp(-s_2) = Supp(s_2) subset Z_1 inter Z_2$となるので$s_1 in Gamma_(Z_1 inter Z_2) (X, cal(F))$として取ってくればいい。\
  $beta$が全射なことを示す。任意に$s in Gamma_(Z_1 union Z_2) (X, cal(F))$をとる。$U_i = X\\Z_i$とおく。$s|_U_1 in Gamma(U_1, cal(F))$と$0 in Gamma(U_2, cal(F))$を考える。これらを$U_1 inter U_2$に制限するとともに$0$となって一致するから，貼り合わせの公理からこれらの拡張$exists t in Gamma(U_1 union U_2, cal(F))$をとることができる。$cal(F)$はflasqueとしたから$s_2 in Gamma(X cal(F))$であって制限が$t$になるものがある。
  $
    s_2|_U_2 = t|_U_2 = 0
  $
  である。$U_2 = X\\Z_2$上で$s_1$は$0$なので$Supp(s_2) subset Z_2$である。$s_1 = s - s_2$とおくと
  $
    s_1|_U_1 = s|_U_1 - s_2|_U_1 = s|_U_1 - s|_U_1 = 0
  $
  だから同じく$Supp(s_1) subset Z_1$である。そこで$(s_1, s_2) in Gamma_(Z_1) (X, cal(F)) plus.o Gamma_(Z_2) (X, cal(F))$を考えれば$beta(s_1, s_2) = s_1 + s_2 = s$となり全射である。

  これを用いて次のようにすると所望の完全列を得る。まず$cal(F)$をinjective resolutionとする。
  $
    0 --> cal(F) --> cal(I)^0 --> cal(I)^1 --> dots.c
  $
  $cal(I)$たちはすべてflasqueである。ゆえに次のような図式を得る。
  $
    #align(center, diagram({
      node((-1, -1), [$0$])
      node((-1, 0), [$0$])
      ...
      node((1, 2), [$dots.v$])
      node((2, 2), [$dots.v$])
      node((0, -1), [$Gamma_(Z_1 inter Z_2) (X, cal(I)^0)$])
      node((0, 0), [$Gamma_(Z_1 inter Z_2) (X, cal(I)^1)$])
      node((0, 1), [$Gamma_(Z_1 inter Z_2) (X, cal(I)^2)$])
      node((1, -1), [$Gamma_(Z_1) (X, cal(I)^0) plus.o Gamma_(Z_2) (X, cal(I)^0)$])
      node((1, 0), [$Gamma_(Z_1) (X, cal(I)^1) plus.o Gamma_(Z_2) (X, cal(I)^1)$])
      node((1, 1), [$Gamma_(Z_1) (X, cal(I)^2) plus.o Gamma_(Z_2) (X, cal(I)^2)$])
      node((2, -1), [$Gamma_(Z_1 union Z_2) (X, cal(I)^0)$])
      node((2, 0), [$Gamma_(Z_1 union Z_2) (X, cal(I)^1)$])
      node((2, 1), [$Gamma_(Z_1 union Z_2) (X, cal(I)^2)$])
      edge((0, -2), (0, -1), "->")
      edge((0, -1), (0, 0), "->")
      edge((0, 0), (0, 1), "->")
      edge((0, 1), (0, 2), "->")
...
適当に調整すれば局所コホモロジーにおけるMayer-Vietoris完全列を得る。
